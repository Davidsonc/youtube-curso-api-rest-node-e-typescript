...
export const seed = async (knex: Knex) => {
    ...
    if (Brazil){
        ...
        for (let index = 0; index < citiesToInsert.length; index++) {
            const element = citiesToInsert[index];
            const [result] = await knex(ETableNames.cidade).insert(element);
        }
    } else {
        const citiesToInsert = allCitiesFromBahia.map((nomeDaCidade) => ({
            nome: nomeDaCidade,
        }));

        await knex(ETableNames.cidade).insert(citiesToInsert);
    }
};

const allCitiesFromBahia = [
    'Abaíra (BA)',
    'Abaré (BA)',
    'Acajutiba (BA)',
    'Adustina (BA)',
...
const allCitiesFromBrazil = [
    ...
    'Imbituva (PR)',
    'Imbuia (SC)',
    'Imigrante (Rio Grande do Sul) (RS)',
    'Imperatriz (MA)',
    'Inaciolândia (GO)',
    'Inácio Martins (PR)',
    'Inajá (PE)',
